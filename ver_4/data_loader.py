...
### Add additive white gaussian noise to image ###
def awgn(x, sigma=5.0):
    H, W, C = x.shape
    mean = 0
...
def salt_pepper(x, p=0.05):
    ...
    for col in range(H):
        for row in range(W):
            rand = np.random.rand()
            if(rand < p/2):
                result[col][row] = [0,0,0]
            elif(rand > p/2 and rand < p):
                result[col][row] = [255,255,255]
            else:
                result[col][row] = x[col][row]

    result = result.astype(np.uint8)
    return result

preprocessings = [awgn, speckle, salt_pepper]
