...
class RoadSurfaceNet(object):
    ...
    def get_model(self):
        ...
        conv_1 = Conv2D(8, kernel_size=(3,3), activation='selu', padding='same', kernel_initializer=self.xavier)(inputs)
        conv_1 = Conv2D(8, kernel_size=(3,3), activation='selu', padding='same', kernel_initializer=self.xavier)(conv_1)
        pool_1 = MaxPooling2D(pool_size=(2,2))(conv_1)

        ### First output ###
        ### Size = W * H * 1 ###
        side_1_logits = Conv2D(1, kernel_size=(1,1), padding='same', name='surface_output_1', kernel_initializer=self.xavier)(conv_1)
        side_1 = tf.keras.activations.sigmoid(side_1_logits)

        ### Stage 2 ###
        conv_2 = Conv2D(16, kernel_size=(3,3), activation='selu', padding='same',kernel_initializer=self.xavier)(pool_1)
        conv_2 = Conv2D(16, kernel_size=(3,3), activation='selu', padding='same',kernel_initializer=self.xavier)(conv_2)
        pool_2 = MaxPooling2D(pool_size=(2,2))(conv_2)

        ### Second output ###
        ### Size = W/2 * H/2 * 1 ###
        side_2_logits = Conv2D(1, kernel_size=(1,1), kernel_initializer=self.xavier)(conv_2)
        side_2_up = UpSampling2D(size=(2,2), interpolation='bilinear')(side_2_logits)
        side_2 = tf.keras.activations.sigmoid(side_2_up)
        
        ### Stage 3 ###
        conv_3 = Conv2D(32, kernel_size=(3,3), activation='selu', padding='same', kernel_initializer=self.xavier)(pool_2)
        conv_3 = Conv2D(32, kernel_size=(3,3), activation='selu', padding='same', kernel_initializer=self.xavier)(conv_3)
        conv_3 = Conv2D(32, kernel_size=(3,3), activation='selu', padding='same', kernel_initializer=self.xavier)(conv_3)
        pool_3 = MaxPooling2D(pool_size=(2,2))(conv_3)

        ### Third output ###
        ### Size = W/4 * H/4 * 1 ###
        side_3_logits = Conv2D(1, kernel_size=(1,1), kernel_initializer=self.xavier)(conv_3)
        side_3_up = UpSampling2D(size=(4,4), interpolation='bilinear')(side_3_logits)
        side_3 = tf.keras.activations.sigmoid(side_3_up)
        
        ### Stage 4 ###
        conv_4 = Conv2D(64, kernel_size=(3,3), activation='selu', padding='same', kernel_initializer=self.xavier)(pool_3)
        conv_4 = Conv2D(64, kernel_size=(3,3), activation='selu', padding='same', kernel_initializer=self.xavier)(conv_4)
        conv_4 = Conv2D(64, kernel_size=(3,3), activation='selu', padding='same', kernel_initializer=self.xavier)(conv_4)
        pool_4 = MaxPooling2D(pool_size=(2,2))(conv_4)

        ### Fourth output ###
        ### Size = W/8 * H/8 * 1 ###
        side_4_logits = Conv2D(1, kernel_size=(1,1), kernel_initializer=self.xavier)(conv_4)
        side_4_up = UpSampling2D(size=(8,8), interpolation='bilinear')(side_4_logits)
        side_4 = tf.keras.activations.sigmoid(side_4_up)
        
        ### Stage 5 ###
        conv_5 = Conv2D(128, kernel_size=(3,3), activation='selu', padding='same',kernel_initializer=self.xavier)(pool_4) 
        conv_5 = Conv2D(128, kernel_size=(3,3), activation='selu', padding='same', kernel_initializer=self.xavier)(conv_5)
        conv_5 = Conv2D(128, kernel_size=(3,3), activation='selu', padding='same', kernel_initializer=self.xavier)(conv_5)

        ### Fifth output ###
        ### Size = W/16 * H/16 * 1 ###
        side_5_logits = Conv2D(1, kernel_size=(1,1),kernel_initializer=self.xavier)(conv_5)
...
class SideNet(object):
    ...
    def get_model(self):
        ...
        conv_1 = Conv2D(8, kernel_size=(3,3), activation='selu', padding='same',kernel_initializer=self.xavier)(concat_input)
        conv_1 = Conv2D(8, kernel_size=(3,3), activation='selu', padding='same',kernel_initializer=self.xavier)(conv_1)
        pool_1 = MaxPooling2D(pool_size=(2,2))(conv_1)

        ### First output ###
        side_1_logits = Conv2D(1, kernel_size=(1,1), name=self.name+'_output_1',kernel_initializer=self.xavier)(conv_1)
        side_1 = tf.keras.activations.sigmoid(side_1_logits)
